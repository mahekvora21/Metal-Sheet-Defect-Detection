...
#using StratifiedKFold to handle data imbalance, creating sets with almost equal distribution of classes
def stratified_group_k_fold(
    ...
    if file is not None:
        df = pd.read_csv(file)
    #defaultdict returns a dictionary like object with default values for absent keys(if provided)
    #labels=dictionary where values are sets
    ...
    groups = []
    Y = []
    #k=group_column, v=list of labels in sme group(set converted to a list)
    #randomly choosing images belonging to particular classes for each group
